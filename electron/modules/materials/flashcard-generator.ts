...
export class FlashcardGenerator {
   ...
   */
  static async generate(text: string, count: number = 10): Promise<Flashcard[]> {
    ...
    for (let attempt = 0; attempt <= this.MAX_RETRIES; attempt++) {
      try {
        const response = await AIEngine.generateResponse(
          SYSTEM_PROMPTS.FLASHCARD_GENERATOR,
          userPrompt
        );
  ...
  private static parseResponse(response: string): Flashcard[] {
    ...
    return parsed
      .map((item: any, i: number) => this.normalizeFlashcard(item, i))
   ...
   */
  private static normalizeFlashcard(item: any, index: number): Flashcard | null {
    try {
