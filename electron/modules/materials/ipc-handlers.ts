...
import { EmbeddingEngine } from '../../ai/embeddings';
...
function getActiveProfileId(): number {
  const db = DBClient.getInstance();
  const profile = db.prepare('SELECT id FROM profiles WHERE is_active = 1').get() as { id: number } | undefined;
  if (!profile) throw new Error('No active profile. Please create or select a profile first.');
  return profile.id;
}

function getLibrarySize(profileId: number): number {
  const db = DBClient.getInstance();
  const result = db.prepare(
    'SELECT COALESCE(SUM(file_size_bytes), 0) AS total FROM study_materials WHERE profile_id = ?'
...
async function runProcessingPipeline(materialId: number, filePath: string, fileType: string): Promise<void> {
  const db = DBClient.getInstance();
  ...
  try {
    ...
    const { text, pageCount } = await DocumentProcessor.extractText(filePath, fileType);
    ...
    for (const chunk of chunks) {
      ...
      try {
        const embedding = await EmbeddingEngine.getEmbedding(chunk.content);
    ...
    try {
      ...
      const topicsResponse = await AIEngine.generateResponse(
        'You are an expert content analyst. Extract the key topics from the following text. Return a JSON array of topic strings. ONLY output valid JSON.',
...
export function registerMaterialHandlers(ipcMain: IpcMain): void {

  // ─── material:upload ──────────────────────────────────────────────────────
  ipcMain.handle('material:upload', async (_event, args: { filePath: string; subject?: string }) => {
    try {
      ...
      `).run(
        profileId,
        path.basename(fileName, ext), // title without extension
        fileName,
        ext.replace('.', ''),
        destPath,
        stat.size,
  ...
  // ─── material:get ─────────────────────────────────────────────────────────
  ipcMain.handle('material:get', async (_event, args: { materialId: number }) => {
    try {
      const profileId = getActiveProfileId();
      const db = DBClient.getInstance();
  ...
  // ─── material:delete ──────────────────────────────────────────────────────
  ipcMain.handle('material:delete', async (_event, args: { materialId: number }) => {
    try {
      const profileId = getActiveProfileId();
      const db = DBClient.getInstance();
      ...
      const material = db.prepare(
        'SELECT file_path FROM study_materials WHERE id = ? AND profile_id = ?'
      ).get(args.materialId, profileId) as { file_path: string } | undefined;
  ...
  // ─── material:summarize ───────────────────────────────────────────────────
  ipcMain.handle('material:summarize', async (_event, args: { materialId: number }) => {
    try {
      const profileId = getActiveProfileId();
      const db = DBClient.getInstance();
      ...
      const chunks = db.prepare(`
        ...
        ORDER BY chunk_index ASC
      `).all(args.materialId, profileId) as { content: string }[];
  ...
  ipcMain.handle('material:generate-quiz', async (_event, args: { materialId: number; count?: number }) => {
    try {
      const profileId = getActiveProfileId();
      const db = DBClient.getInstance();
      ...
      const fullText = chunks.map(c => c.content).join('\n\n');
      const questions = await QuizGenerator.generate(fullText, args.count);
  ...
  ipcMain.handle('material:generate-flashcards', async (_event, args: { materialId: number; count?: number }) => {
    try {
      const profileId = getActiveProfileId();
      const db = DBClient.getInstance();
      ...
      const fullText = chunks.map(c => c.content).join('\n\n');
      const flashcards = await FlashcardGenerator.generate(fullText, args.count);
